...
def parallel_douglas_rachford_gpu(tau, inits, n_iter, inner_params, ds):
    """
    ...
    for t in range(n_iter):
        ...
        for start in range(0, ds.N, batch_size):
            ...
            # 更新对偶变量 v
            v_t[rng_tensor] = v_t[rng_tensor] + tau * (grad_F(u_batch, ds) - grad_F(u_t, ds))
        
        # Step 3: 更新全局变量 u (闭式解)
        u_t = (1.0 / (ds.alpha * ds.N)) * (ds.x_hat + v_t.sum(dim=0))
        
        # 评估
        acc, loss = evaluate_model(u_t, ds)
        hist_acc.append(acc)
        hist_loss.append(loss)
        print(f"  DR Iter {t+1}: Loss={loss:.4f}, Acc={acc:.2f}%")
    
    elapsed = time.time() - start_time
    print(f"DR Finished on {device}. Time: {elapsed:.2f}s\n")
    
    # 将结果移回 CPU
    return u_t.cpu(), hist_acc, hist_loss
...
def baseline_sgd(ds, n_epochs=30, lr=0.01, momentum=0.9, batch_size=64):
    ...
    for epoch in range(n_epochs):
        ...
        # 评估
        with torch.no_grad():
            acc, total_loss = evaluate_model(w.detach(), ds)
            hist_acc.append(acc)
            hist_loss.append(total_loss)
            print(f"  SGD Epoch {epoch+1}: Loss={total_loss:.4f}, Acc={acc:.2f}%")
    
    elapsed = time.time() - start_time
    print(f"SGD Finished on {device}. Time: {elapsed:.2f}s\n")
    return w.detach().cpu(), hist_acc, hist_loss
...
if __name__ == "__main__":
    ...
    # 初始化 (在 GPU 上)
    u0 = torch.zeros(K, d, device=device)       # (K, d) 全局权重初始化
    v0 = torch.zeros(N, K, d, device=device)    # (N, K, d) 对偶变量初始化
    inits = Inits(u0, v0)
    
    # 内层参数
    inner_params = Params(n_iter=10, lr=0.01)
    
    # 运行 DR 算法 (GPU 加速)
    w_dr, hist_acc_dr, hist_loss_dr = parallel_douglas_rachford_gpu(
        tau=0.3,              # 步长参数
        inits=inits,
        n_iter=N_ITER,
    ...
    w_sgd, hist_acc_sgd, hist_loss_sgd = baseline_sgd(
        ds,
        n_epochs=N_ITER,
        lr=0.01,
        momentum=0.9,
    ...
    # 准确率对比
    axes[0].plot(hist_acc_dr, 'b-o', label='Parallel DR (Ours)', linewidth=2, markersize=4)
    axes[0].plot(hist_acc_sgd, 'r--s', label='Baseline SGD', linewidth=2, markersize=4)
    axes[0].set_xlabel('Iteration', fontsize=12)
    axes[0].set_ylabel('Accuracy (%)', fontsize=12)
    axes[0].set_title(f'Accuracy Comparison (N={N_SAMPLES}, Device={device})', fontsize=14)
    axes[0].legend(fontsize=11)
    axes[0].grid(True, alpha=0.3)
    
    # 损失对比
    axes[1].plot(hist_loss_dr, 'b-o', label='Parallel DR (Ours)', linewidth=2, markersize=4)
    axes[1].plot(hist_loss_sgd, 'r--s', label='Baseline SGD', linewidth=2, markersize=4)
    axes[1].set_xlabel('Iteration', fontsize=12)
    axes[1].set_ylabel('Loss', fontsize=12)
    axes[1].set_title(f'Loss Comparison (N={N_SAMPLES}, Device={device})', fontsize=14)
    ...
    print(f"Parallel DR   - Final Acc: {hist_acc_dr[-1]:.2f}%, Final Loss: {hist_loss_dr[-1]:.4f}")
    print(f"Baseline SGD  - Final Acc: {hist_acc_sgd[-1]:.2f}%, Final Loss: {hist_loss_sgd[-1]:.4f}")
    print("="*60)
